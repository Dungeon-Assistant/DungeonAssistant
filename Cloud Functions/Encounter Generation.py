...
#This code loosely based on the code from https://github.com/jforstneric/5e_encounter_generator. It is published under the MIT license and is free for open or closed source projects.
thresholds = [
	[25, 50, 75, 100, 500],
	[50, 100, 150, 200, 1100],
	[75, 150, 225, 400, 1700],
...
def load_monsters(type):
    ...
    #When empty type, load the whole database. 
    if(type == ""):
        dataurl = 'https://api.open5e.com/monsters/?limit=500'
    else:
        #Otherwise, make smaller calls of the given type
        dataurl = 'https://api.open5e.com/monsters/?type=' + type
    while dataurl != None:
        #continuously pull until we hit the end of the database, adding mons to a list. 
...
def encounter_gen(monsterList, xpThreshold):
    ...
    while xpMonsters < (xpThreshold - (3 * xpLowerLimit)):		# keep adding monsters until we get close enought to xpThreshold
        possibleMonsters = []
        for m in monsterList:		# filter out monsters with too high or too low xp values
        ...
        if not possibleMonsters:
            ...
            return encounteredMonsters #Return what we did obtain.
            
        #Get a random possible monster from our list.
...
#print out the monsters in a nicely formated way using HTML formatting. 
...
def main(dict):
    #Get the values needed from our dictionary
    numPcs = int(dict.get('size', -1))
    avgLev = int(dict.get('level', -1))
    difficulty = dict.get('difficulty', 'Easy')
    encounterChoice = dict.get('monsterType', "")
    
    #Check for an encounter Choice
    if(encounterChoice != None):
        encounterChoice = encounterChoice.lower()
    else:
        encounterChoice = ""
    #Calculate difficulty multiplier.
    difficulty = diffToMult.get(difficulty)
    
    #Calculate EXP threshold
    xp = party_threshold(numPcs,avgLev,difficulty)
    #Load monsters
    monster_data = load_monsters(encounterChoice)
    #Build the encounter, and design the string
    encounteredMonsters = encounter_gen(monster_data, xp)
    val = print_encounter(encounteredMonsters)
